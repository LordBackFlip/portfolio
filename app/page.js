...
export default function Home() {
  ...
  useEffect(() => {
    fetchCategories().then(async (data) =>  await setCategories(data.data));
  }, []);

  useEffect(() => {
    categories.forEach((category) => {
      fetchGames(category.name).then(async (data) =>
        await setGamesByCategory((prev) => ({ ...prev, [category.name]: data.data.list }))
      );
    });
  }, [categories]);
